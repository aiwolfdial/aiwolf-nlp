...
以下は一般向けの大会レギュレーション説明です。エージェント実装を行う参加者の方は、 [エージェントの作成と対戦方法](/menu/AIWolfDial2026_SpringJp/agent)に技術的詳細がありますのでご覧ください。
...
- 予選は主にエージェントの動作確認を目的として実施します。予選と本戦とでエージェントのシステムや実装が異なっていても問題ありませんが、本戦での接続不備や想定外の挙動を避けるため、予選サーバを用いて確実に接続および挙動をご確認ください。
- 予選で生成されたログは、他チームが閲覧・利用する可能性があります。予選ログを分析し、エージェントの改善やモデル学習等に活用することは、レギュレーション上問題ありません。
...
- 会話パート（talk）では自然言語での対話を行います。プロトコルなど自然言語以外の使用は禁止です。発話しない場合はSkip、当該のdayで発話をしない場合はOverを返します（プロトコル部門と同じ）。
...
9人村: [aiwolf-nlp-server/config/default_9.yml](https://github.com/aiwolfdial/aiwolf-nlp-server/blob/main/config/default_9.yml#L16)
...
従来のトークフェーズではサーバがエージェントの順番を管理し、1エージェントずつ発話を待機する方式でした。
いつでも発話トラックでは、エージェントが任意のタイミングで自ら発話できる方式に変わります。

- フェーズの制限時間は10分、1エージェントあたりの最大発話回数は4回です
- 連続発話の最小間隔は設けていません
- 全エージェントがOverを送信するか、残り発話回数が0になった場合、制限時間前にフェーズが終了します
- 発話上限を超えた場合はエラーにはならず、サーバ側で破棄されます

プロトコルの詳細は[エージェントの作成と対戦方法](/menu/AIWolfDial2026_SpringJp/agent#いつでも発話トラックへの対応)をご確認ください。