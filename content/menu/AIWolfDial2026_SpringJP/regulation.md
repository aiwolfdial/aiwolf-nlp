...
従来のトークフェーズではサーバがエージェントの順番を管理し、1エージェントずつ `TALK` リクエストを送信して発話を待機する方式でした。新たなグループチャットモードでは、エージェントが任意のタイミングで自ら発話できる方式に変わります。

- サーバはトークフェーズ開始時に `TALK_PHASE_START` リクエスト（仮称）、終了時に `TALK_PHASE_END` リクエスト（仮称）を送信
- エージェントはその間、任意のタイミングで発話レスポンスを送信可能
- 他エージェントの発話は `TALK_BROADCAST` リクエスト（仮称）で1発話ごとに配信（レスポンス不要）

※制限時間や連続発話の最小間隔等は検討中です。対応サーバ・雛形エージェント・ビューアは4月中を目処に公開予定です。