...
こちらのページでは、`config.yml` の `prompt` セクションを編集するために必要な知識をまとめます。
...
独自のキーを追加したい／特定のタイミングで別のプロンプトに差し替えたい場合は、`agent.py` にメソッドを足してプロンプトを自分で呼び出す必要があります。コード側の具体的な手順は、姉妹ページの [エージェントのカスタマイズ方法 ＞ 独自キーのプロンプトを追加する](./customize_agent.md#独自キーのプロンプトを追加する) を参照してください（こちらのページでは、既存リクエストに対してプロンプトがどう使われるかに絞って説明します）。
