...
こちらのページでは、本プロジェクトで **APIキーの管理** に使われている **環境変数** と、その実体である **`.env` ファイル** について説明します。
...
## `.env` は「隠しファイル」なので見えにくい

`.env` の先頭の `.` は、Unix の **隠しファイル（ドットファイル）** の印です。そのため `ls` には出てこず、`ls -a` で初めて見えます。
隠しファイルの概念とコマンドでの扱い（`ls -a` / `cat` / dotディレクトリの注意など）は [ターミナル操作とコマンド ＞ 隠しファイル（ドットファイル）](./about_terminal.md#隠しファイルドットファイル) にまとめてあります。ここでは `.env` 特有の「見つからない」トラブルだけ補足します。

> **`.env` が見当たらないとき**：`ls -a config/` で表示されるか確認してください。Windows のエクスプローラでは「隠しファイルを表示」を有効にしないと見えません。

> **空ファイルにしか見えない／保存できないとき**：Windows のメモ帳が拡張子 `.txt` を勝手に付け、`env.txt` になっていることがあります。ファイル名が正確に `.env` か確認してください。
